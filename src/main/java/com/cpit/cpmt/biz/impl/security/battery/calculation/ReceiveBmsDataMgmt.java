...
public class ReceiveBmsDataMgmt {
	...
	public void CollectSingleChargeData(CheckedBMS checkedBMS) {
		logger.info("CheckedBMS信息解析[{}]", JSON.toJSONString(checkedBMS));
		logger.info("checkedBMS中operatorID=[{}],connectorID=[{}],bmsCode=[{}],startTime=[{}]",
				checkedBMS.getOperatorID(), checkedBMS.getConnectorID(), checkedBMS.getBmsCode(),
	...
	// List<BmsHot> bmsHotList = mongoTemplate.find(new Query(criteria),
	// BmsHot.class, "bmsHot");
	// List<BmsInfo> bmsInfoList = new ArrayList<BmsInfo>();
	// for (BmsHot bmsHot : bmsHotList) {
	// BmsInfo bmsInfo = new BmsInfo();
